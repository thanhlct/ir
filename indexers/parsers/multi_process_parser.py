...
class MultiProcessParser(object):
    ...
    def parse_one_block(self, name, id_maker, from_id, to_id, result_q):
        id_maker = id_maker.get_sub_id_maker(from_id, to_id)
        
        for doc_id, doc_des in id_maker:
            print '%s: %d/%d'%(name, doc_id, to_id)

    def parse_an_id_maker_old(self, id_maker):
        self.time_stat.start_clock('parser')
        self.time_stat.start_clock('block')
        processed = 0
        for doc_id, doc_des in id_maker:
